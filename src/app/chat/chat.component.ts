...
export class ChatComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, 
    ...
    private router: Router
      ...
      this.socket = io('http://192.168.100.113:3000', {
 ...
 this.socket.on('newMessage', (newMessage: string) => {
  ...
  fetchChatMessages() {    
    ...
    this.http.get<any>('http://192.168.100.113:3000/chat-rooms/' + this.room, {
  ...
  sendMessage() {
    ...
    this.http.post<any>('http://192.168.100.113:3000/chat-rooms/' + this.room + '/messages', messageData, {
      headers: new HttpHeaders().set('Authorization', 'Bearer ' + localStorage.getItem('token'))
    })
      .subscribe(
        (response) => {
